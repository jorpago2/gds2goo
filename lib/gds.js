...
export function parseGds(buffer) {
  ...
  while (offset + 4 <= view.byteLength) {
    ...
    if (length < 4 || length % 2 !== 0 || offset + length > view.byteLength) {
    ...
    if (type === RECORD.UNITS && end - start >= 16) {
      ...
      if (structure || element) throw new Error("A GDS structure started before the previous structure ended.");
      structure = { name: "", elements: [] };
    } else if (type === RECORD.STRNAME) {
      if (!structure || element) throw new Error("A GDS structure name appeared outside a structure header.");
      structure.name = ascii(new Uint8Array(buffer, start, end - start));
    } else if (type === RECORD.ENDSTR) {
      if (!structure) throw new Error(`GDS ENDSTR appeared without an active structure at byte ${offset}.`);
      if (element) throw new Error(`GDS structure “${structure.name || "(unnamed)"}” ended before its element ended.`);
      if (!structure.name) throw new Error("A GDS structure without a name was found.");
      if (structures.has(structure.name)) throw new Error(`Duplicate GDS structure name “${structure.name}”.`);
      structures.set(structure.name, structure);
      structure = null;
    } else if ([RECORD.BOUNDARY, RECORD.PATH, RECORD.SREF, RECORD.AREF, RECORD.TEXT, RECORD.NODE, RECORD.BOX].includes(type)) {
      if (!structure || element) throw new Error("A GDS element started before the previous element ended.");
      element = {
        kind: type,
        layer: 0,
        datatype: 0,
      ...
      if (element.absoluteWidth) absoluteWidthCount += 1;
      ...
      if ((end - start) % 8 !== 0) throw new Error(`Invalid GDS XY record at byte ${offset}.`);
      element.points = [];
    ...
    } else if (type === RECORD.ENDEL) {
      if (!element || !structure) throw new Error(`GDS ENDEL appeared without an active element at byte ${offset}.`);
  ...
  if (offset !== view.byteLength || structure || element) throw new Error("The GDS stream ended before all records were closed.");
  ...
  if (absoluteWidthCount) warnings.push(`${absoluteWidthCount} PATH element(s) use absolute WIDTH; its physical width is preserved through references.`);
