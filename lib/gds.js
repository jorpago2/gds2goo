...
export function parseGds(buffer) {
  ...
  while (offset + 4 <= view.byteLength) {
    ...
    if (length < 4 || offset + length > view.byteLength) {
      throw new Error(`Registro GDS inválido en el byte ${offset}.`);
    }
    ...
    if (type === RECORD.UNITS && end - start >= 16) {
      ...
      if (!structure.name) throw new Error("Se encontró una estructura GDS sin nombre.");
  ...
  if (records === 0 || structures.size === 0) throw new Error("El archivo no contiene estructuras GDSII legibles.");
...
export function flattenGds(model, topCell) {
  ...
  function visit(name, matrix, stack) {
    if (stack.length > 64) throw new Error("La jerarquía GDS supera 64 niveles.");
    if (stack.includes(name)) throw new Error(`Referencia GDS circular: ${[...stack, name].join(" → ")}.`);
    const structure = model.structures.get(name);
    if (!structure) throw new Error(`La celda referenciada “${name}” no existe.`);
  ...
  if (!output.length) throw new Error(`La celda “${topCell}” no contiene BOUNDARY, BOX ni PATH rasterizables.`);
  return output;
}
