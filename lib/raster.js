...
/** Merge an exposed overlay into a mask while preserving the selected polarity. */
export function mergeBinaryOverlay(pixels, overlayPixels, inverted = false) {
  if (pixels.length !== overlayPixels.length) throw new Error("Overlay and mask dimensions must match.");
  const overlayValue = inverted ? 0 : 1;
  for (let index = 0; index < pixels.length; index += 1) {
    if (overlayPixels[index]) pixels[index] = overlayValue;
  }
  return pixels;
}
