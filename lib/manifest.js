...
export function createRunManifest({ source, mask, exposures, process, outputs }) {
  if (!source || typeof source !== "object" || !mask || typeof mask !== "object"
    || !Array.isArray(outputs) || !outputs.length || outputs.some((name) => typeof name !== "string" || !name.trim())
    || !Array.isArray(exposures) || !exposures.length) {
    throw new Error("The run manifest is incomplete.");
  }
  const normalizedExposures = exposures.map((value) => Number(value));
  if (normalizedExposures.some((value) => !Number.isFinite(value) || value < 0.1 || value > 600)) {
    throw new Error("Manifest exposures must be between 0.1 and 600 s.");
  }
  const safeProcess = process && typeof process === "object" && !Array.isArray(process) ? process : {};
  const text = (value) => typeof value === "string" ? value.trim() : "";
  const thicknessNanometers = text(safeProcess.thicknessNm) ? Number(safeProcess.thicknessNm) : null;
  if (thicknessNanometers !== null && !(Number.isFinite(thicknessNanometers) && thicknessNanometers > 0)) {
    throw new Error("Photoresist thickness must be a positive finite number.");
  }
  return {
    ...
    mask,
    exposuresSeconds: normalizedExposures,
    process: {
      photoresist: text(safeProcess.photoresist) || null,
      thicknessNanometers,
      softBake: text(safeProcess.softBake) || null,
      development: text(safeProcess.development) || null,
      notes: text(safeProcess.notes) || null,
    },
    outputs,
  };
}
