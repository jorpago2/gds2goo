...
export function encodeBinaryLayer(readRows, width, height) {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    if (row.length !== width) throw new Error(`La fila ${y} tiene ${row.length} píxeles; se esperaban ${width}.`);
...
function writePreview(writer, pixels, width, height) {
  if (pixels && pixels.length !== width * height) throw new Error("Vista previa GOO con tamaño inválido.");
...
export function buildGooFile({ layerData, exposureSeconds, whitePixels = 0, smallPreview, bigPreview }) {
  if (!(exposureSeconds > 0 && exposureSeconds <= 600)) throw new Error("La exposición debe estar entre 0 y 600 s.");
  ...
  if (writer.offset !== HEADER_SIZE) throw new Error(`Cabecera GOO inválida: ${writer.offset} bytes.`);
  ...
  if (writer.offset !== writer.bytes.length) throw new Error("Longitud GOO interna inconsistente.");
  return writer.bytes;
}
...
export function validateGooFile(bytes, expectedPixels = MARS_4_9K.width * MARS_4_9K.height) {
  ...
  for (let i = 0; i < MAGIC.length; i += 1) if (bytes[4 + i] !== MAGIC[i]) throw new Error("Firma GOO incorrecta.");
  ...
  if (dataEnd + 14 !== bytes.length) throw new Error("Longitud de capa GOO incorrecta.");
  if (bytes[dataEnd] !== checksumOf(bytes.subarray(dataStart, dataEnd))) throw new Error("Checksum de capa GOO incorrecto.");
  ...
  if (pixels !== expectedPixels) throw new Error(`La capa GOO decodifica ${pixels} píxeles; se esperaban ${expectedPixels}.`);
